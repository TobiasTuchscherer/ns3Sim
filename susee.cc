...
void OnTxPowerChange (double oldTxPower, double newTxPower)
{
  ...
  std::cout << "tx Power:" << newTxPower << std::endl;
}


// Trace sources  fired when a PHY layer begins transmitting a packet
//void BeginPacket (Ptr<const Packet> packet, uint32_t start)
//{
//std::cout << "fired when a PHY layer begins transmitting a packet" << std::endl;
//}
...
//  int nGateways = 1;
   ...
   LogComponentEnable ("susee", LOG_LEVEL_ALL);
...
//  std::cout << tracker.CountMacPacketsGlobally (Seconds (0), appStopTime + Hours (1)) << std::endl;
  ...
  Time simulationTime = Seconds (900 * nPeriods);
  Simulator::Stop (simulationTime);
  Simulator::Run ();

  Simulator::Destroy ();

  std::cout << tracker.CountMacPacketsGlobally(Seconds (900 * (nPeriods - 2)),
                                               Seconds (900 * (nPeriods - 1))) << std::endl;
